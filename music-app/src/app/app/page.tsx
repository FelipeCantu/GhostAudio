...
type SectionId = "welcome" | "stats" | "recentlyPlayed" | "recentlyAdded" | "quickActions";
...
const STORAGE_KEY = "dizc-dashboard-order-v1";
...
function SortableSection({ id, isEditing, children }: SortableSectionProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <div ref={setNodeRef} style={style}>
        ...
        {isEditing && (
          ...
          >
            <button
              {...attributes}
              {...listeners}
              className="p-2 text-zinc-500 hover:text-zinc-300 cursor-grab active:cursor-grabbing touch-none min-w-[44px] min-h-[44px] flex items-center justify-center"
      ...
      <div
        className={`transition-all duration-200 ${
          isEditing ? "pl-11 pointer-events-none select-none opacity-60" : ""
        }`}
      >
        {children}
      </div>
...
export default function Home() {
  ...
  // Section order — initialised from localStorage, falls back to default
  const [sectionOrder, setSectionOrder] = useState<SectionId[]>(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        // Ensure all default sections are present (graceful migration)
        const merged = [
            ...
            {recentFive.map((entry, i) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() =>
                    playTrack(entry.track, undefined, entry.albumInfo ?? undefined)
                  }
            ...
            return (
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-white/8 flex items-center justify-center group-hover:bg-[#f4d35e]/15 transition-colors">
                  <Icon
                    size={18}
                    className="text-zinc-400 group-hover:text-[#f4d35e] transition-colors"
                  />
                </div>
                <div>
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all min-h-[44px] min-w-[44px] ${
              isEditing
                ? "bg-[#f4d35e] text-[#0d3b66] hover:bg-[#f0c830] shadow-lg shadow-[#f4d35e]/20"
                : "bg-white/6 border border-white/8 text-zinc-400 hover:text-white hover:bg-white/10"
            }`}
            aria-label={isEditing ? "Done customizing" : "Customize dashboard layout"}
        ...
        <AnimatePresence>
          {isEditing && (
            ...
            >
              Drag the handles to reorder sections. Changes are saved automatically.
            </motion.p>
          )}
        </AnimatePresence>
          ...
          <SortableContext items={sectionOrder} strategy={verticalListSortingStrategy}>
            <div className="space-y-6 md:space-y-8">
              {sectionOrder.map((id) => {
                ...
                // recentlyPlayed section is null when there's no history — skip it entirely
                if (content === null) return null;
                return (
                  <SortableSection key={id} id={id} isEditing={isEditing}>
                    {content}
                  </SortableSection>
                );
              })}
